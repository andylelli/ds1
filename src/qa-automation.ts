...
async function runQA(expectedMode: 'simulation' | 'live') {
  ...
  try {
    ...
    if (expectedMode === 'simulation') {
      if (config.useSimulatedEndpoints !== true) console.error('❌ Config mismatch: useSimulatedEndpoints should be true in Sim mode');
      else console.log('✅ Sidebar Config: Simulation options enabled');
    } else {
      if (config.useSimulatedEndpoints !== false) console.error('❌ Config mismatch: useSimulatedEndpoints should be false in Live mode');
      else console.log('✅ Sidebar Config: Simulation options disabled (Correct)');
    }
  } catch (e: any) {
  ...
  if (expectedMode === 'simulation') {
    if (startRes.status === 200) console.log('✅ /simulation/start: Allowed (200 OK)');
    else console.error(`❌ /simulation/start: Failed (${startRes.status})`);
  } else {
    if (startRes.status === 403) console.log('✅ /simulation/start: Blocked (403 Forbidden) - Correct for Live Mode');
    else console.error(`❌ /simulation/start: Should be 403, got ${startRes.status}`);
  }

  // Test Clear
  const clearRes = await fetch(`${API_URL}/simulation/clear`, { method: 'POST' });
  if (expectedMode === 'simulation') {
    if (clearRes.status === 200) console.log('✅ /simulation/clear: Allowed (200 OK)');
    else console.error(`❌ /simulation/clear: Failed (${clearRes.status})`);
  } else {
    if (clearRes.status === 403) console.log('✅ /simulation/clear: Blocked (403 Forbidden) - Correct for Live Mode');
    else console.error(`❌ /simulation/clear: Should be 403, got ${clearRes.status}`);
  }
  ...
  try {
    ...
    if (chatRes.status === 200) {
      ...
      if (expectedMode === 'simulation') {
        if (isMockResponse || chatData.response.includes('operational')) console.log('✅ Verified: Received Mock response');
        else console.warn('⚠️ Warning: Expected Mock response, got potential real response?');
      } else {
        if (!isMockResponse) console.log('✅ Verified: Received Live AI response');
        else console.error('❌ Error: Received Mock response in Live mode!');
      }
    } else {
