export function calculateLinearRegression(y: number[]): { slope: number; intercept: number; r2: number } {
    const n = y.length;
...
export type TrendShape = 'Rising' | 'Peaking' | 'Falling' | 'Flat' | 'Volatile';

export function analyzeTrendShape(points: number[]): TrendShape {
    if (!points || points.length < 5) return 'Flat'; // Not enough data
    ...
    if (slope1 > 0.5 && slope2 < -0.5) return 'Peaking';
