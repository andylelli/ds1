...
// Mock Dependencies
const mockDb: any = { saveActivityLog: async () => {}, getPriorLearnings: async () => [] };
const mockEventBus: any = { subscribe: async () => {}, publish: async () => {} };
const mockTrendAnalyzer: any = {};
const mockCompetitorAnalyzer: any = {};

async function runTest() {
    const agent = new ProductResearchAgent(mockDb, mockEventBus, mockTrendAnalyzer, mockCompetitorAnalyzer);
    
    // Mock Brief
    const brief: any = { execution_speed: 'normal' }; // 14 days

    // Scenario A: Rising Trend (Slope > 0.5)
    const themeRising: any = { 
        id: 't1', 
        name: 'Rising Star', 
    ...
    // Inject signals
    (agent as any).collectedSignals = [signalRising, signalFalling, signalWinter];
    ...
    const passedIds = passed.map((t: any) => t.id);
