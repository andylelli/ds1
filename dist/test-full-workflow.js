...
async function runFullWorkflow() {
    ...
    // Research -> CEO
    eventBus.subscribe('PRODUCT_FOUND', 'CEO_Reviewer', async (payload) => {
        console.log(`[Event] PRODUCT_FOUND: ${payload.product.name}`);
        await ceo.review_product(payload);
    });
    // CEO (Approve Product) -> Supplier
    eventBus.subscribe('PRODUCT_APPROVED', 'Supplier_Finder', async (payload) => {
    ...
    eventBus.subscribe('SUPPLIER_FOUND', 'CEO_Supplier_Reviewer', async (payload) => {
    ...
    eventBus.subscribe('SUPPLIER_APPROVED', 'Store_Builder', async (payload) => {
    ...
    // Store -> Marketing
    eventBus.subscribe('PRODUCT_PAGE_CREATED', 'Marketing_Campaigner', async (payload) => {
    ...
    // Marketing -> Done
    eventBus.subscribe('CAMPAIGN_STARTED', 'Workflow_Monitor', async (payload) => {
    ...
    eventBus.subscribe('ORDER_RECEIVED', 'Operations_Manager', async (payload) => {
        console.log(`[Event] ORDER_RECEIVED: ${payload.order.id}`);
        await operationsAgent.process_order(payload);
    });
    // Customer Service -> Done
    eventBus.subscribe('ORDER_SHIPPED', 'CS_Agent', async (payload) => {
    ...
    // Analytics
    eventBus.subscribe('DAILY_REPORT_REQUESTED', 'Analytics_Engine', async (payload) => {
