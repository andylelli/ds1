...
export class ProductResearchAgent extends BaseAgent {
     ...
     */
    async createResearchBrief(requestId, criteria) {
        const systemPrompt = `
     ...
     */
    async ingestPriorLearnings(brief) {
     ...
     */
    async checkTimeFitness(themes, brief) {
     ...
     * Section 8: Deep Validation
     */
    async performDeepValidation(themes) {
     ...
     * Section 9: Productization
     */
    async createOfferConcepts(themes) {
     ...
     */
    async createOpportunityBriefs(concepts, themes, requestId, researchBrief) {
     ...
     */
    async handleResearchRequest(payload) {
        ...
        try {
            ...
            await this.eventBus.publish('OpportunityResearch.PriorLearningsAttached', {
                brief_id: briefId,
            ...
            await this.eventBus.publish('OpportunityResearch.ThemesGenerated', {
                brief_id: briefId,
                themes: themes
            }, request_id);
            ...
            await this.eventBus.publish('OpportunityResearch.ThemesGated', {
                brief_id: briefId,
            ...
            await this.eventBus.publish('OpportunityResearch.ShortlistRanked', {
                brief_id: briefId,
            ...
            await this.eventBus.publish('OpportunityResearch.TimeFiltered', {
                brief_id: briefId,
            ...
            await this.eventBus.publish('OpportunityResearch.ValidatedCandidatesReady', {
                brief_id: briefId,
            ...
            await this.eventBus.publish('OpportunityResearch.OfferConceptsCreated', {
                brief_id: briefId,
            ...
            await this.eventBus.publish('OpportunityResearch.BriefsPublished', {
                brief_id: briefId,
            ...
            for (const brief of briefs) {
                ...
                await this.eventBus.publish('Supplier.FeasibilityRequested', {
                    brief_id: briefId,
                    opportunity_id: brief.id,
                    concept: brief.concept
                }, request_id);
                // 2. Request Marketing Angle Analysis
                await this.eventBus.publish('Marketing.AngleWhitespaceRequested', {
                    brief_id: briefId,
                    opportunity_id: brief.id,
                    concept: brief.concept
                }, request_id);
    ...
    async generateSearchStrategies(userInput) {
        try {
            const client = openAIService.getClient();
            const response = await client.chat.completions.create({
                model: openAIService.deploymentName,
